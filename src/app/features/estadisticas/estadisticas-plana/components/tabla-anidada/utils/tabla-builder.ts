...
interface NomData {
  total: number;
  apelaciones: Record<string, ApeData>;
}
interface MesData {
  total: number;
  nomenclaturas: Record<string, NomData>;
}
interface AnioData {
  total: number;
...
function _salaChart(salaData: SalaData): ChartSlice[] {
  return Object.entries(salaData.anios).map(([anioStr, anioData]) => ({
    label: anioStr,
    value: anioData.total,
  }));
}
...
function _buildSalaRows(salaData: SalaData): { rows: TableRowRich[]; totalGeneral: number } {
  ...
  for (const [anioStr, anioData] of Object.entries(salaData.anios)) {
    ...
    const anioChart: ChartSlice[] = Object.entries(anioData.meses).map(([mesStr, mesData]) => ({
      label: mesStr,
      value: mesData.total,
    }));
  ...
  for (const [mesStr, mesData] of Object.entries(anioData.meses)) {
    ...
    if (Object.keys(mesData.nomenclaturas).length > 1) {
      mesRows.push({
        ...
        total: mesData.total,
        chartData: mesChart,
        chartTitle: `${sala} | ${anioStr} | ${mesStr}`,
        _sala: sala,
        _anio: anioStr,
        _mes: mesStr,
      });
...
function _buildMesRows(
  ...
  for (const [nomStr, nomData] of Object.entries(mesData.nomenclaturas)) {
    const nomChart: ChartSlice[] = Object.entries(nomData.apelaciones).map(([apeStr, apeData]) => ({
      label: apeStr,
      value: apeData.total,
    }));
    ...
    if (Object.keys(nomData.apelaciones).length > 1) {
      nomRows.push({
        ...
        total: nomData.total,
        chartData: nomChart,
        chartTitle: `${sala} | ${anioStr} | ${mesStr} | ${nomStr}`,
        _sala: sala,
        _anio: anioStr,
        _mes: mesStr,
        _nom: nomStr,
...
function _buildNomRows(
  ...
  for (const [apeStr, apeData] of Object.entries(nomData.apelaciones)) {
    const apeChart: ChartSlice[] = Object.entries(apeData.tipos).map(([tipoStr, tipoData]) => ({
      label: tipoStr,
      value: tipoData.total,
    }));
    const apeTitle = `${sala} | ${anioStr} | ${mesStr} | ${nomStr} | ${apeStr}`;

    const apeRows = _buildApeRows(
      sala,
      anioStr,
      mesStr,
      nomStr,
