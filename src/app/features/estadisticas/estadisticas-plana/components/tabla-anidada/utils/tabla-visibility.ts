...
export function applyVisibility(rows: TableRowRich[], collapsed: Set<string>): TableRowRich[] {
  ...
  for (const row of rows) {
    ...
    if (row.salaRowspan) { p.cSala = row; p.cSala._calcSalaSpan = 0; p.cSala.salaColapsado = collapsed.has(`S:${row._sala}`); }
    if (row.anioRowspan) { p.cAnio = row; p.cAnio._calcAnioSpan = 0; p.cAnio.anioColapsado = collapsed.has(`A:${row._sala}|${row._anio}`); }
    if (row.mesRowspan)  { p.cMes  = row; p.cMes._calcMesSpan   = 0; p.cMes.mesColapsado   = collapsed.has(`M:${row._sala}|${row._anio}|${row._mes}`); }
    if (row.nomRowspan)  { p.cNom  = row; p.cNom._calcNomSpan   = 0; p.cNom.nomColapsado   = collapsed.has(`N:${row._sala}|${row._anio}|${row._mes}|${row._nom}`); }
    if (row.apeRowspan)  { p.cApe  = row; p.cApe._calcApeSpan   = 0; p.cApe.apeColapsado   = collapsed.has(`P:${row._sala}|${row._anio}|${row._mes}|${row._nom}|${row._ape}`); }
